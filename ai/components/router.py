...
class Router:
    ...
    @classmethod
    async def index_path(cls, user_input: str) -> Coroutine[Any]:
        ...
        if response_schema["message_type"] == "help":
            ...
            if not any(response_schema.values()):
                return QAIndicesManager.qa_query(response_schema)
            return QAIndicesManager.qa_based_query(response_schema)

        elif message_type == "summarize":
            return SummaryIndicesManager.summary_query(response_schema)
